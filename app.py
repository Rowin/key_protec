...
from forms import AffectationForm, CSRFBase, room_form_builder
...
@app.route("/users/<int:user_id>/deaffect", methods=["POST"])
@db_session
def deaffect(user_id: int):
    ...
    form: CSRFBase = CSRFBase(request.form)
    if form.validate():
        user.card = None
...
def list_users():
    users = User.select()
    form: AffectationForm = AffectationForm(request.form)
    csrf_form: CSRFBase = CSRFBase(request.form)
    return render_template("users_listing.html", users=users, form=form, csrf_form=csrf_form)
